...
public class BinarySearchTreeOperation {
	...
	public void allPairsWithGivenSum(BinarySearchTreeNode root, int target) {
		ArrayList<BinarySearchTreeNode> leftList = new ArrayList<>();
		ArrayList<BinarySearchTreeNode> rightList = new ArrayList<>();

		BinarySearchTreeNode curr_left = root, curr_right = root;

		while (curr_left != null || curr_right != null || leftList.size() > 0 && rightList.size() > 0) {

			while (curr_left != null) {
				leftList.add(curr_left);
				curr_left = curr_left.getLeft();
			}

			while (curr_right != null) {
				rightList.add(curr_right);
				curr_right = curr_right.getRight();
			}

			var leftNode = leftList.get(leftList.size() - 1);
			var rightNode = rightList.get(rightList.size() - 1);

			int leftVal = leftList.get(leftList.size() - 1).getData();
			int rightVal = rightList.get(rightList.size() - 1).getData();
			...
			if (leftVal + rightVal < target) {
				...
				curr_left = leftNode.getRight();
			}

			else if (leftVal + rightVal > target) {
				rightList.remove(rightList.size() - 1);
				curr_right = rightNode.getLeft();
			}

			else {
				System.out.println(leftNode.getData() + " " + rightNode.getData());
